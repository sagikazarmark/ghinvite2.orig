...
case "$target" in
  integration_test|admission_protocol_proof|authoritative_admission|durable_projection|retained_delivery) ;;
  *) printf '%s\n' 'Usage: bash scripts/test-restate.sh [integration_test|admission_protocol_proof|authoritative_admission|durable_projection|retained_delivery]' >&2; exit 2 ;;
esac
