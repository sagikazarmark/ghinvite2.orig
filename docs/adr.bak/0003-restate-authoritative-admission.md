...
**Target architecture update (2026-09-24):** [ADR 0009](0009-canonical-link-identity-and-request-owned-lifecycle.md) replaces the link-owned request lifecycle/dispatch and workflow notification arrangement through a clean prelaunch redesign. The rules below remain historical context and describe the current implementation; the new spec must preserve the stated replay and eligibility guarantees through its explicit request/admission protocol.
...
The #48 [split-state proof](https://github.com/sagikazarmark/ghinvite2.orig/blob/f78d8d2/crates/ghinvite-workflows/tests/admission_protocol_proof/split_state.rs) (at `f78d8d2`, since retired in favour of `authoritative_admission`) verified multi-key interruption recovery and these pointer rules; `interruption_recovery` and `lifecycle_recovery` in `crates/ghinvite-workflows/tests/authoritative_admission.rs` now exercise them against the production handlers. The original aggregate proof below retains evidence for admission clock/dispatch and SQL outage. Exclusive authoritative reads may wait during a stuck link mutation; asynchronous database projection still does not hold that mutation open. Administrative kill/purge remains outside ordinary replay recovery and requires a repair procedure before reopening the object to commands.

### Durable transition and dispatch: verified bounded sequence

The #48 protocol proof ([`crates/ghinvite-workflows/tests/admission_protocol_proof.rs` at `f78d8d2`](https://github.com/sagikazarmark/ghinvite2.orig/blob/f78d8d2/crates/ghinvite-workflows/tests/admission_protocol_proof.rs), since retired in favour of `authoritative_admission`) exercised this sequence on Restate 1.7.9 with Rust SDK 0.10.0. `crates/ghinvite-workflows/tests/authoritative_admission.rs` now holds the production handlers to it, with fault checkpoints between the steps (`interruption_recovery`):
...
The proof results below are historical. The proof and its `admission_protocol_proof` runner target were retired after [`f78d8d2`](https://github.com/sagikazarmark/ghinvite2.orig/tree/f78d8d2/crates/ghinvite-workflows/tests), which is where to read them. Its scenarios are now covered against production handlers by `crates/ghinvite-workflows/tests/authoritative_admission.rs` (checkpoint interruption, final-use race, replay/409, SQL outage and revocation, expiry and deadlines, lazy state, notification replay and conflict), `durable_projection.rs` (acknowledgement loss and ordered convergence), `retained_delivery.rs` (late notification after workflow completion), and the `ghinvite-core` storage test suite (stale snapshots cannot regress a revoked link).

Command: `bash scripts/test-restate.sh admission_protocol_proof` (at `f78d8d2`) — **passed**, one bounded scenario, 8.32 seconds after compilation/runtime startup. The existing `bash scripts/test-restate.sh` gate also passed, as did targeted Clippy with `-D warnings` and shell syntax validation.
...
The same command, extended with [`admission_protocol_proof/split_state.rs`](https://github.com/sagikazarmark/ghinvite2.orig/blob/f78d8d2/crates/ghinvite-workflows/tests/admission_protocol_proof/split_state.rs) (at `f78d8d2`), **passed in 12.70 seconds** including the original aggregate/outage scenarios. It registers a separate test-only object with lazy state enabled and exclusive status reads, using the same runtime, SDK, native request-response endpoint, and scratch SQL schema.
...
`bash scripts/test-restate.sh admission_protocol_proof` (at `f78d8d2`) **passed in 18.84 seconds**, including the earlier aggregate and split-key scenarios and the new [deadline extension](https://github.com/sagikazarmark/ghinvite2.orig/blob/f78d8d2/crates/ghinvite-workflows/tests/admission_protocol_proof/deadlines.rs). `lifecycle_boundaries`, `lifecycle_recovery`, `overdue_status_and_timer_race`, `overdue_readmission`, `interrupted_timer_setup` and `queued_expiry` in `authoritative_admission` now cover these scenarios on the production handlers. It uses the same pinned native request-response runtime and scratch SQL projector.
...
`bash scripts/test-restate.sh admission_protocol_proof` (at `f78d8d2`) **passed in 21.96 seconds**, including all earlier scenarios and [notifications.rs](https://github.com/sagikazarmark/ghinvite2.orig/blob/f78d8d2/crates/ghinvite-workflows/tests/admission_protocol_proof/notifications.rs). `authoritative_workflow_contract` in `authoritative_admission` now covers interrupted resolution, identical replay and the 409 for a conflicting terminal signal on the production workflow; `retained_delivery` covers late notification after completion. Targeted Clippy with `-D warnings` passed. The proof configures two-second completion retention on its workflow and a one-second cleanup scan on the disposable smoke runtime only. The pinned runtime otherwise scans hourly, with an initial delay: simply waiting two seconds did not exercise cleanup. See pinned [cleaner implementation](https://github.com/restatedev/restate/blob/v1.7.9/crates/worker/src/partition/cleaner.rs).
