...
|---|------|----------|--------|----------|------------|
| 1 | **Foundations** | `2026-05-04-ghinvite-foundations.md` | Written + autoplan-amended | `crates/domain` (types, IDs, slug, state machines), `crates/audit` (event types), `crates/storage` (trait + sqlx impl), migrations, parameterized test suite | none |
| 2 | **GitHub clients** | _not yet written_ | Pending | `crates/github` — user-OAuth client (oauth2 crate), installation-token client (JWT signing + token cache), HMAC verification helper, mock transport for tests | 1 |
