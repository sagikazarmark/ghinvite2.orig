...
- Workers Static Assets must be configured with `run_worker_first` covering all SSR routes. `not_found_handling = "single-page-application"` must **not** be used: on `compatibility_date >= 2025-04-01` it serves `index.html` for every unmatched navigation and would hijack `/`, `/i/{slug}`, and `/login`.
