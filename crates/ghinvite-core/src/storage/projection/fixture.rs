...
/// Link-only projection; mutable request state has a separate writer.
pub fn envelope(link: &InvitationLink, revision: u64) -> ProjectionEnvelope {
    ProjectionEnvelope {
...
pub async fn seed<S: ProjectionStorage + ?Sized>(
    s: &S,
    link: &InvitationLink,
    requests: &[InvitationRequest],
    revision: u64,
) -> crate::storage::Result<()> {
    s.apply_transition(&envelope(link, revision)).await?;
    for request in requests {
        s.apply_request(&RequestProjectionEnvelope {
            request: request_snapshot(link, request, revision),
            events: vec![],
        })
        .await?;
    }
...
pub fn request_snapshot(
    ...
    RequestSnapshot {
        ...
        decision_deadline,
        revision,
        decision: (request.state != RequestState::Pending).then(|| TerminalDecision {
            decision_id: format!("fixture/{}", request.id),
            decided_by: request.decided_by,
            effective_at: request.decided_at.unwrap_or(request.created_at),
            evaluated_at: request.decided_at.unwrap_or(request.created_at),
    ...
    S: crate::storage::Storage + ProjectionStorage + ?Sized,
{
    ...
    seed(s, &link, std::slice::from_ref(request), revision).await
}
    ...
    S: crate::storage::Storage + ProjectionStorage + ?Sized,
{
    let link = s
        ...
        .ok_or(crate::storage::Error::NotFound)?;
    s.apply_request(&RequestProjectionEnvelope {
        request: request_snapshot(&link, request, DECISION_REVISION),
        events: vec![],
    })
    .await
}
