...
//! The Workers entry point in `crates/ghinvite-workflows-worker` wraps the same
...
//! - `GHINVITE_LISTEN_ADDR`            — bind address (default: 127.0.0.1:9080); use
//!   0.0.0.0:9080 when Restate must reach this binary from inside Docker
//! - `GHINVITE_DATABASE_PATH`          — path to SQLite file (default: in-memory, resets on restart)
//! - `RESTATE_IDENTITY_KEY`            — Restate Cloud signing key (`publickeyv1_...`);
//!   optional for local dev, required in production

use std::net::SocketAddr;
use std::sync::Arc;

use ghinvite_github::{InstallationClient, jwt::AppJwtSigner, transport::ReqwestTransport};
use ghinvite_workflows::{AppState, build_endpoint};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let github_client = if let Some(pem) = pem {
        ...
        let dummy_pem = include_str!("../../ghinvite-github/src/jwt_test_key.pem");
    ...
    tracing::info!("ghinvite-workflows listening on http://{addr}");
    tracing::info!(
